...
BEGIN
  ...
  IF prediction_record.predicted_winner = race_record.winner THEN
...
-- Add some initial prediction data for testing (you can remove this later)
-- First, let's create some sample predictions for existing users
DO $$
DECLARE
  sample_user_id uuid;
  sample_race_id uuid;
...
BEGIN
  -- Get a race ID
  SELECT id INTO sample_race_id FROM races LIMIT 1;
  
  -- Get some driver IDs
  SELECT array_agg(id) INTO driver_ids FROM drivers LIMIT 5;
  
  -- Create some sample predictions with realistic points
  -- User 1: Good predictions
  INSERT INTO user_predictions (user_id, race_id, predicted_winner, predicted_podium, points_earned)
  VALUES (
    gen_random_uuid(),
    sample_race_id,
    driver_ids[1],
    driver_ids[1:3],
    43  -- Winner + podium bonus
  );
  
  -- User 2: Decent predictions  
  INSERT INTO user_predictions (user_id, race_id, predicted_winner, predicted_podium, points_earned)
  VALUES (
    gen_random_uuid(),
    sample_race_id,
